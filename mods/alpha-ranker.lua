...
local function scale_basic_stat(unit, stat, factor)
    if unit[stat] then
...
local function has_stat(unit, stat, t)
...
local function scale_stats(unit, stat_set, factor)
    for stat, t in pairs(stat_set) do
        if has_stat(unit, stat, t) then
            ...
            if stat_type == 1 then
                scale_basic_stat(unit, stat, mod)
            elseif stat_type == 2 then
                scale_weapon_stat(unit, stat, mod)
...
local function faulty(unit)
    -- -20% stats
    scale_stats(unit, stats, 0.8)
end

local function common(unit)
    -- do nothing
end

local function uncommon(unit)
    -- +20% stats
    scale_stats(unit, stats, 1.2)
end

local function rare(unit)
    -- +50% stats
    scale_stats(unit, stats, 1.5)
end
...
local function corrupted(unit)
    ...
    for i=1,n do
        ...
        scale_stats(unit, choice, 10)
    end
end

local function mythic(unit)
    -- +100% stats
    scale_stats(unit, stats, 2)
end

local function speedy(unit)
    -- all speed-like stats + 100%
    scale_stats(unit, speed_stats, 2)
end
...
local modifier_p_table = {  -- probability weights 210 total
    {faulty, 20}, -- 9.5%
    {common, 100}, -- 47.6%
    {uncommon, 50}, -- 23.8%
    {rare, 20}, -- 9.5%
